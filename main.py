# -*- coding: utf-8 -*-
"""
pitchine.py - Final version for a dedicated backend Web Service.
This version contains the server-side Google OAuth2 flow and the core
application logic, but does not serve any static frontend files.
"""
...
async def auth_via_google(request: Request):
    try:
        ...
        if user_info and user_info.get('email_verified'):
            ...
            # Redirect back to the FRONTEND domain, passing the custom token
            frontend_url = f"https://pitchine.com/?logintoken={custom_token.decode('utf-8')}"
            return RedirectResponse(url=frontend_url)
            
    except Exception as e:
        print(f"ERROR during Google auth callback: {e}")
        return RedirectResponse(url="https://pitchine.com/?error=auth_failed")
    
    return RedirectResponse(url="https://pitchine.com/?error=unknown")
